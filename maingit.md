...
- [Основы работы с удаленным репозитарием](#--------------------------------------)
  ...
  * [git push — вносим изменения в удаленный репозиторий](#git-push-------------------------------------------)
- [Работа с локальным репозиторием](#-------------------------------)
  * [Базовые команды](#---------------)
    + [git init — создание репозитория](#git-init-----------------------)
    + [git add и git rm — индексация изменений](#git-add---git-rm-----------------------)
    + [git status — состояние проекта, измененные и не добавленные файлы, индексированные файлы](#git-status------------------------------------------------------------------------------)
    + [git commit — совершение коммита](#git-commit---------------------)
    + [git reset — возврат к определенному коммиту, откат изменений, «жесткий» или «мягкий»](#git-reset---------------------------------------------------------------------------)
    + [git revert — отмена изменений, произведенных в прошлом отдельным коммитом](#git-revert---------------------------------------------------------------)
    + [git log — разнообразная информация о коммитах в целом](#git-log----------------------------------------------)
    + [git diff — отличия между деревьями проекта, коммитами и т.д.](#git-diff--------------------------------------------------)
    + [git show — показать изменения, внесенные отдельным коммитом](#git-show---------------------------------------------------)
    + [git blame и git annotate — команды, помогающие отслеживать изменения файлов](#git-blame---git-annotate---------------------------------------------------)
    + [git grep — поиск слов по проекту, состоянию проекта в прошлом](#git-grep-----------------------------------------------------)
  * [Ветвление](#---------)
    + [git branch — создание, перечисление и удаление веток](#git-branch------------------------------------------)
    + [git checkout — переключение между ветками, извлечение файлов](#git-checkout------------------------------------------------)
    + [git merge — слияние веток (разрешение возможных конфликтов)](#git-merge--------------------------------------------------)
    + [git rebase — построение ровной линии коммитов](#git-rebase-----------------------------------)
    + [git cherry-pick — применение к дереву проекта изменений, внесенных отдельным коммитом](#git-cherry-pick----------------------------------------------------------------------)
  * [Прочие команды и необходимые возможности](#----------------------------------------)
    + [Хэш — уникальная идентификация объектов](#---------------------------------------)
    + [git tag — тэги как способ пометить уникальный коммит](#git-tag---------------------------------------------)
    + [Относительная адресация](#-----------------------)
    + [файл .gitignore — объясняем git, какие файлы следует игнорировать](#-----gitignore-------------git----------------------------------)
